...
def compute_scene_aware_clusters(
    ...
    """Cluster keyframes by (EE position, gripper state, object position).

    Two keyframes are in the same cluster only if ALL three match:
    ...
    Args:
        ...
        obj_radius: max object distance for same cluster.
...
def collect_alt_targets(
    ...
    """Collect alternative next-targets for truly ambiguous keyframes.

    A keyframe gets alt targets only when it belongs to a scene-aware
    cluster (same EE + grip + object position) where members have
    divergent next-targets (distance > target_diverge_threshold).
