...
def create_replay_temporal(
    ...
    extra_replay_elements = [
        ...
        # Multi-peak: alternative targets for truly ambiguous keyframes
        ReplayElement("alt_target_positions", (multipeak_max_peaks, 3), np.float32),
        ReplayElement("alt_target_mask", (multipeak_max_peaks,), np.bool_),
    ]
...
def _add_keypoints_to_replay(
    ...
    for k in range(
        ...
        # Multi-peak alt targets for this keyframe
        if ep_alt_positions is not None and ep_alt_mask is not None:
            kf_alt_pos = ep_alt_positions[k]    # (max_peaks, 3)
            kf_alt_msk = ep_alt_mask[k]         # (max_peaks,)
        else:
            kf_alt_pos = np.zeros((5, 3), dtype=np.float32)
            kf_alt_msk = np.zeros((5,), dtype=np.bool_)
        ...
        others = {
            ...
            "alt_target_positions": kf_alt_pos,
            "alt_target_mask": kf_alt_msk,
        }
        final_obs = {
...
def fill_replay(
    ...
    # Load precomputed multi-peak targets if provided
    multipeak_targets = None
    if multipeak_targets_json and os.path.exists(multipeak_targets_json):
        import json as _json
        with open(multipeak_targets_json) as _f:
            multipeak_targets = _json.load(_f)
        print(f"[Info] Loaded multi-peak targets: {multipeak_targets_json}")
    ...
    if disk_exist:
        ...
        for d_idx in range(start_idx, start_idx + num_demos):
            ...
            # Multi-peak alt targets: load from precomputed JSON or compute on-the-fly (intra only)
            num_kf = len(episode_keypoints)
            ep_alt_positions = None
            ep_alt_mask = None
            if multipeak_targets is not None and str(d_idx) in multipeak_targets:
                mp = multipeak_targets[str(d_idx)]
                ep_alt_positions = np.array(mp["alt_positions"], dtype=np.float32)[:num_kf]
                ep_alt_mask = np.array(mp["alt_mask"], dtype=bool)[:num_kf]
            else:
                # Fallback: compute intra-episode only
                from sam2act.mvt.multipeak_utils import collect_alt_targets
                kf_poses = np.array([
                    demo[episode_keypoints[k]].gripper_pose for k in range(num_kf)
                ], dtype=np.float32)
                kf_opens = np.array([
                    demo[episode_keypoints[k]].gripper_open for k in range(num_kf)
                ], dtype=np.float32)
                ep_alt_positions, ep_alt_mask = collect_alt_targets(
                    kf_poses, kf_opens, num_kf, max_peaks=3,
                )
            ...
            for i in range(len(demo) - 1):
                ...
                _add_keypoints_to_replay(
                    ...
                    device=device,
                    ep_alt_positions=ep_alt_positions,
                    ep_alt_mask=ep_alt_mask,
                )
...
def _add_keypoints_to_replay_temporal(
    ...
    for k in range(
        ...
        if k == 0:
            ...
            keypoint_frame = episode_keypoints[k - 1]
        # Multi-peak alt targets
        if ep_alt_positions is not None and ep_alt_mask is not None:
            kf_alt_pos = ep_alt_positions[k]
            kf_alt_msk = ep_alt_mask[k]
        else:
            kf_alt_pos = np.zeros((5, 3), dtype=np.float32)
            kf_alt_msk = np.zeros((5,), dtype=np.bool_)
...
def fill_replay_temporal(
    ...
    if disk_exist:
        ...
        for d_idx in range(start_idx, start_idx + num_demos):
            ...
            # Load alt targets for this episode
            num_kf = len(episode_keypoints)
            ep_alt_positions = None
            ep_alt_mask = None
            if multipeak_targets is not None and str(d_idx) in multipeak_targets:
