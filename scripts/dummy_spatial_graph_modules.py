...
def main():
    ...
    bank = SpatialGraphMemoryBank(distance_threshold=0.08, ema_alpha=0.5)
    memory_bank = []
    for idx in range(seq):
        ...
        memory_bank, _, _ = bank.update(memory_bank, feat, pos_enc, pos3d, idx)

    query_pos = torch.randn(1, 3, device=device)
    bias = build_spatial_attention_bias(
        query_pos=query_pos,
        memory_nodes=memory_bank[: min(len(memory_bank), 8)],
