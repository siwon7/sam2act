...
EPOCHS="${6:-20}"
SAME_AUG_PER_SEQ="${7:-false}"
TRANSFORM_AUG="${8:-true}"
AUTO_EVAL_EPISODES="${9:-0}"
ATTN_EPISODE="${10:-0}"
NUM_MASKMEM="${11:-9}"
...
case "${SAME_AUG_PER_SEQ,,}" in
  true) SAME_AUG_PER_SEQ="True" ;;
  false) SAME_AUG_PER_SEQ="False" ;;
esac

case "${TRANSFORM_AUG,,}" in
  true) TRANSFORM_AUG="True" ;;
  false) TRANSFORM_AUG="False" ;;
esac
...
echo "task=$TASK exp_name=$EXP_NAME bs=$BS lr=$LR epochs=$EPOCHS same_aug_per_seq=$SAME_AUG_PER_SEQ transform_aug=$TRANSFORM_AUG auto_eval_episodes=$AUTO_EVAL_EPISODES attn_episode=$ATTN_EPISODE num_maskmem=$NUM_MASKMEM" | tee -a "$LOG_FILE"
...
torchrun --standalone --nproc_per_node=4 train_plus.py \
  ...
  --exp_cfg_opts "tasks ${TASK} exp_name ${EXP_NAME} wandb False bs ${BS} epochs ${EPOCHS} peract.lr ${LR} peract.same_trans_aug_per_seq ${SAME_AUG_PER_SEQ} peract.transform_augmentation ${TRANSFORM_AUG}" \
...
if [[ "$AUTO_EVAL_EPISODES" -gt 0 ]]; then
  ...
  EVAL_CSV="$RUN_DIR/eval/$EVAL_LOG_NAME/model_plus_last/eval_results.csv"
  if [[ -f "$EVAL_CSV" ]]; then
    echo "eval_results_csv=$EVAL_CSV" | tee -a "$LOG_FILE"
    tail -n +1 "$EVAL_CSV" | tee -a "$LOG_FILE"
  else
    echo "Missing eval CSV: $EVAL_CSV" | tee -a "$LOG_FILE"
  fi
fi
