...
ROOT=/home/cv25/siwon/sam2act_role_graph_memory
REPO=$ROOT/sam2act
BASELINE=/home/cv25/siwon/sam2act/sam2act
RUN_NAME=${RUN_NAME:-pbb_role_graph_smoke_$(date +%Y%m%d_%H%M%S)}
...
torchrun --standalone --nproc_per_node=1 train_plus.py \
  ...
  --exp_cfg_opts "tasks put_block_back exp_name ${RUN_NAME} wandb False bs 12 epochs 1 num_workers 0 train_iter 120 peract.same_trans_aug_per_seq True peract.phase_aux_loss_weight 0.1 peract.phase_aux_num_classes 4 peract.role_graph_loss_weight 0.05 peract.role_ref_loss_weight 0.02 peract.anchor_use_loss_weight 0.01 peract.role_contrastive_loss_weight 0.01" \
  --mvt_cfg_opts "num_maskmem 11 role_graph_enabled True role_graph_num_classes 6 phase_graph_num_classes 4 role_graph_bias_scale 0.05 anchor_use_bias_scale 0.02 persistent_anchor_enabled True persistent_anchor_max_steps 2 persistent_anchor_prepend True"
