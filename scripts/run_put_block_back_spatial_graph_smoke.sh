...
torchrun --standalone --nproc_per_node=2 train_plus.py \
  ...
  --exp_cfg_opts "tasks put_block_back exp_name ${EXP_NAME} wandb False demo ${DEMO_COUNT} bs ${BS} epochs ${EPOCHS} train_iter ${TRAIN_ITER} num_workers 0 peract.lr 1.25e-06 peract.same_trans_aug_per_seq False peract.transform_augmentation True peract.spatial_graph_contrastive_loss_weight 0.05 peract.spatial_graph_contrastive_temp 0.1 peract.spatial_graph_contrastive_thresh 0.08" \
  --mvt_cfg_opts "num_maskmem 11 use_memory True spatial_graph_enabled True spatial_graph_dist_thresh 0.08 spatial_graph_ema_alpha 0.5 spatial_graph_mask_value -10000.0 spatial_graph_write_use_gt_train True" \
